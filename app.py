...
df = pd.read_csv("E:/IMDB_movies-main/IMDB-Movie-Data.csv")
...
g_list = list(set(','.join(list(df.Genre)).split(',')))

st.write(g_list)

if menu == 'Home':
     image = 'movies-disk.png'
     msg = '''Hey there,<br> this is my project 'IMDB movie data Analytics'.<br>
 ...
 """, unsafe_allow_html = True)
     ...
     st.write('  ')
     st.write(df)
     st.markdown(f""" <hr> <div style="display: grid; grid-template-columns: 1fr 1fr;">
...
if menu == menu_items[1]:
    ...
    fig = px.pie(names=g_list, values=mov_count)
    st.plotly_chart(fig)
    st.write('Maximum movies have Genre \'Drama\'')
    bar_data = pd.DataFrame({'Genre':g_list, 'Total Revenue':genre_rev, 'Total Votes':genre_vote, 'Average Rating':genre_rate})
